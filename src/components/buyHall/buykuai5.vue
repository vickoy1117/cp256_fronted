    ...
    <div class="buykuai3">
        ...
        <div class="buyHallDetail">
            ...
            <div class="choseList" v-for="(item,init) in selectGroup">
                ...
                <ul class="out">
                    <li class="n" v-for="(number,index) in item.options" @click="selectgroup(index,init)" :keyname="item.keyname" v-if="!(playedId == 260)">
                        <div :class="number.class">{{number.num}}</div>
                        <div v-if="isFixed == 1">{{number.odds}}</div>
                    </li>
                    <!--趣味型-定单双(260)为4字的号超出边框，故单独设计-->
                    <li class="n" v-for="(number,index) in item.options" @click="selectgroup(index,init)" :keyname="item.keyname" v-if="playedId == 260">
    ...
    methods: {
        ...
        selectgroup: function (index, init) {
            ...
            function tz11x5Select() {
                if (_self.playedId == 46) {
                    // 至少选2个数
                    combineSelect(2)
                } else if (_self.playedId == 47) {
                    // 至少选3个数
                    combineSelect(3)
                } else if (_self.playedId == 48) {
                    // 至少选4个数
                    combineSelect(4)
                } else if (_self.playedId == 49) {
                    // 至少选5个数
                    combineSelect(5)
                } else if (_self.playedId == 50) {
                    // 至少选6个数
                    combineSelect(6)
                } else if (_self.playedId == 51) {
                    // 至少选7个数
                    combineSelect(7)
                } else if (_self.playedId == 52) {
                    // 至少选8个数
                    combineSelect(8)
                } else {
                    // 至少选1个数
            ...
            function combineSelect(num) {
                if (_self.arr[0] instanceof Array) {
                    if (_self.arr[0].length > (num - 1)) {
                        _self.total = combine(_self.arr[0], num).length;
                    } else {
                        _self.total = 0;
                    }
                } else {
